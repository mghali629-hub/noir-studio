  ...
  return (
    <header className="sticky top-0 z-40 bg-black/90 backdrop-blur-xl border-b border-zinc-800 font-mono">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
        ...
        <nav className="hidden lg:flex items-center gap-4 text-xs font-sans font-medium text-zinc-300 overflow-x-auto no-scrollbar [ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden py-2">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
